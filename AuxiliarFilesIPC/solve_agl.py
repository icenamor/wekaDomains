...
if __name__ == '__main__':
    ...
    default_timeouts = [15, 30, 45, 45, 45, 45, 20]
    ...
    if((accumulated_time < timelimit) and ((counter == 1) or ((counter > 1) and (not optimal_planning)))):
        ...
        if((accumulated_time < timelimit) and ((counter == 1) or ((counter > 1) and (not optimal_planning)))):
            planners2 =  ["fd-autotune-1" ,"lama-2011","fd-autotune-2"]
            time_total = []
            for i in range(len(planners2)):
        	    timeFinal = int(math.ceil(timelimit - accumulated_time)/ (len(planners2)*2))
